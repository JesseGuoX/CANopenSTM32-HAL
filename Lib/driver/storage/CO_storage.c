...
static ODR_t OD_write_6200(OD_stream_t *stream, const void *buf,
                           OD_size_t count, OD_size_t *countWritten)
{
    /* verify arguments */
    ...
    case 1:
        (data > 0)?HAL_GPIO_WritePin(D1_GPIO_Port, D1_Pin, GPIO_PIN_RESET):HAL_GPIO_WritePin(D1_GPIO_Port, D1_Pin, GPIO_PIN_SET);
        break;
    case 2:
        (data > 0)?HAL_GPIO_WritePin(D2_GPIO_Port, D2_Pin, GPIO_PIN_RESET):HAL_GPIO_WritePin(D2_GPIO_Port, D2_Pin, GPIO_PIN_SET);
        break;
    case 3:
        (data > 0)?HAL_GPIO_WritePin(D3_GPIO_Port, D3_Pin, GPIO_PIN_RESET):HAL_GPIO_WritePin(D3_GPIO_Port, D3_Pin, GPIO_PIN_SET);
        break;
    case 4:
        (data > 0)?HAL_GPIO_WritePin(D4_GPIO_Port, D4_Pin, GPIO_PIN_RESET):HAL_GPIO_WritePin(D4_GPIO_Port, D4_Pin, GPIO_PIN_SET);
        break;
    }

    if (returnCode == ODR_OK) *countWritten = sizeof(uint8_t);
    return returnCode;
}
                                 ...
                                 uint8_t entriesCount)
{
    ...
    if(OD_6200_LED != NULL){
        storage->OD_6200_extension.object = storage;
        storage->OD_6200_extension.read = OD_readOriginal;
        storage->OD_6200_extension.write = OD_write_6200;
        OD_extension_init(OD_6200_LED,
                          &storage->OD_6200_extension);
    }
